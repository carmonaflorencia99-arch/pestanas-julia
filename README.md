...
y alertas de salud.
Roles
Secretaria: busca clientas, da de alta clientas nuevas, ve el historial completo, arma la Agenda de hoy (asigna qué profesional atiende a cada clienta).
Profesional: al entrar ve directamente su cola del día en una vista tipo tablet (tarjetas grandes, táctiles) con solo las clientas que la secretaria le asignó hoy. Desde ahí carga la ficha de servicio.
Admin: todo lo anterior + gestión de personal, catálogo de servicios y exportación a CSV.
Novedad: Agenda del día + vista tablet
Cada clienta puede tener una profesional habitual (preferencia general, se elige al crear la clienta).
Cada día, la secretaria arma la cola de hoy desde la pestaña "Agenda de hoy": elige clienta + profesional + hora.
Esa profesional, al iniciar sesión desde su tablet, ve solo esas clientas asignadas, en tarjetas grandes pensadas para tocar de pie, sin buscador ni menús adicionales.
La profesional puede marcar cada clienta como "En curso" y luego "Completar", y abrir la ficha a pantalla completa para cargar el servicio.
Recuerda ejecutar `schema_v2_migracion.sql` en el SQL Editor de Supabase (después del `schema.sql` original) para habilitar esta función.
---
1. Configurar Supabase (una sola vez)
Crea una cuenta gratuita en supabase.com y un proyecto nuevo.
Ve a SQL Editor → pega y ejecuta el contenido completo de `schema.sql`.
...
Copia el UUID del usuario recién creado (aparece en la lista de usuarios).
Ve a SQL Editor y ejecuta (cambiando los datos):
```sql
insert into staff (auth_user_id, nombre, email_auth, rol)
values ('PEGA-AQUI-EL-UUID', 'Julia', 'julia@laspestanasdejulia.local', 'admin');
```
A partir de aquí, esa admin puede dar de alta al resto del personal desde el
...
En vercel.com, pulsa Add New → Project y selecciona el repo.
Vercel detecta Vite automáticamente. Antes de desplegar, ve a
Environment Variables y añade:
`VITE_SUPABASE_URL`
`VITE_SUPABASE_ANON_KEY`
Pulsa Deploy. En 1-2 minutos tendrás una URL como `pestanas-julia.vercel.app`.
(Opcional) En Settings → Domains puedes conectar un dominio propio,
